...
export default function Home(){
 ...
 <section className="proof-section"><Eyebrow>THE ARCHITECTURE PACKAGES</Eyebrow><h2>Two chips. <em>A deck and a film for each.</em></h2><div className="dr-pkg-cards">{packages.map(p=><article className="dr-pkg-card" key={p.id}><button className="dr-pkg-poster" onClick={()=>go('library?pkg='+p.id)} aria-label={'Watch the '+p.name+' architecture film'}><img src={p.poster} alt="" loading="lazy" width={1280} height={720}/><span className="dr-play"><Play size={20} fill="currentColor"/></span></button><div className="dr-pkg-body"><span className="mono">{p.name} · {p.slides.length} SLIDES · {fmtTime(p.duration)} FILM</span><h3>{p.headline}</h3><p>{p.summary}</p><div className="dr-pkg-actions"><button className="primary" onClick={()=>go('library?pkg='+p.id)}>Watch and browse <ArrowUpRight size={17}/></button><a className="text-link" href={p.deck} download>Download .pptx <Download size={16}/></a></div></div></article>)}</div><div className="proof-bottom"><p>Client-ready PowerPoint decks, narrated films and draw.io diagrams, built from Deepgrid Semi’s architecture documents, datasheets and block diagrams. {PRE_SILICON}</p><button className="text-link" onClick={()=>navigate('library')}>All decks and films <ArrowUpRight size={18}/></button></div></section>
 </>}

 {view==='library'&&<section className="page-wrap"><SectionHead tag="02 / DECKS & FILMS" title="Architecture decks, films and diagrams" copy="One package per chip: a client-ready PowerPoint deck, a narrated film of the same slides, and the draw.io diagram behind them."/><Library pkgId={route.params.get('pkg')||'lite'} slide={Number(route.params.get('slide'))||1} onChange={update}/></section>}
 ...
 {view==='architecture'&&<section className="page-wrap"><SectionHead tag="03 / ARCHITECTURE" title="Faults are contained, never silent" copy="Every block on DG32-LITE is a 32-bit register slave on one deterministic bus. A bad access returns an error instead of hanging, and a CPU disagreement raises a pin."/>
  ...
  <div className="dr-figure-head"><p className="eyebrow"><span/> THE FULL DIAGRAM</p><button className="text-link" onClick={()=>go('library?pkg=lite')}>Watch the DG32-LITE architecture film <ArrowUpRight size={17}/></button></div><div className="figure-scroll dr-figure"><img src="./diagrams/dg32-lite-architecture.svg" alt="DG32-LITE system architecture diagram: safety core, memory and boot, supervision, on-chip bus, motor drive, sensing and math, connectivity and test, with the numbered current-control loop and the fault path" loading="lazy"/></div>
 ...
 {view==='pinout'&&<section className="page-wrap"><SectionHead tag="05 / PINOUT & PACKAGE" title="44 signals in a 9 × 9 mm package" copy="The 64-pin QFN carries every signal a brushless drive needs; the remaining 20 pins are supplies and grounds. DG32-2DOM uses the identical pinout."/>
  <div className="dr-pinout"><figure className="dr-pin-figure"><button className="dr-slide-link" onClick={()=>go('library?pkg=lite&slide=12')} aria-label="Open slide 12 of the DG32-LITE architecture deck"><img src="./decks/dg32-lite/slide-12.webp" alt="DG32-LITE architecture deck, slide 12: 44 signals and a hardware trip in 9 × 9 mm" loading="lazy" width={1600} height={900}/></button><figcaption>Slide 12 of the DG32-LITE architecture deck. Pin map awaiting the foundry’s bond-diagram confirmation.</figcaption></figure>
 ...
 <footer className="footer"><div className="footer-top"><button className="brand" onClick={()=>navigate('overview')} aria-label="DeepGrid Semi home"><Brand/></button><h2>Safety in the core.<br/><em>Control in silicon.</em></h2><a className="text-link" href="https://deepgridsemi.com" target="_blank" rel="noreferrer">deepgridsemi.com <ArrowUpRight size={20}/></a></div><div className="footer-bottom"><span>© 2026 DEEPGRID SEMI PVT LTD</span><span>HYDERABAD · INDIA</span><span>PRE-SILICON · DESIGN VALUES, NOT MEASUREMENTS</span><a href="#library" onClick={e=>{e.preventDefault();navigate('library')}}>Decks & films ↗</a></div></footer>
