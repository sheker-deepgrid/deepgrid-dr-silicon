...
function Trace({step,onJump}:{step:number;onJump:(i:number)=>void}){
 ...
 return <div className="dr-trace" aria-hidden="true" onPointerMove={onMove}>
  ...
  <div className="tr-cores">
   <div className={'tr-node tr-interactive'+on(0)+active(0)} onClick={()=>onJump(0)} role="button" tabIndex={0} title="Step 01: MAIN core commits fault"><span className="mono">MAIN CORE</span><strong>Runs the application</strong><em className="tr-val tr-bad">wrong value</em></div>
   <div className={'tr-node tr-interactive'+on(1)+active(1)} onClick={()=>onJump(1)} role="button" tabIndex={0} title="Step 02: CHECKER core commits correct value"><span className="mono">CHECKER · 2 CYCLES LATER</span><strong>Same inputs, mirrored</strong><em className="tr-val tr-good">correct value</em></div>
  </div>
  <div className={'tr-node tr-cmp tr-interactive'+on(2)+active(2)} onClick={()=>onJump(2)} role="button" tabIndex={0} title="Step 03: Hardware Comparator detects mismatch"><span className="mono">COMPARATOR</span><strong>{step>=2?'Mismatch on this store':'Checks every committed store'}</strong><b className="tr-ne">≠</b></div>
  <div className={'tr-node tr-latch tr-interactive'+on(3)+active(3)} onClick={()=>onJump(3)} role="button" tabIndex={0} title="Step 04: Sticky Fault Latch locks cause"><span className="mono">FAULT LATCH</span><strong>{step>=3?'First cause held':'Clear'}</strong></div>
  <div className={'tr-node tr-bridge tr-interactive'+on(4)+active(4)} onClick={()=>onJump(4)} role="button" tabIndex={0} title="Step 05: Gate driver bridge forced flat in hardware"><span className="mono">FAULT_N · GATE DRIVER</span><strong>{step>=4?'Bridge off, in hardware':'Bridge switching'}</strong><span className="tr-gates">{[0,1,2,3,4,5].map(i=><i key={i}/>)}</span><small>{step>=4?'Within 39 cycles of an injected fault, simulated':'Six gate outputs active'}</small></div>
 </div>;
}
